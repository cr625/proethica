...
if [ "$CODESPACES" != "true" ]; then
    ...
    if ! command -v docker &> /dev/null; then
        ...
        if [ -z "$CONTAINER_STATUS" ]; then
            ...
            echo -e "${YELLOW}If using Docker for PostgreSQL, please create the container with:${NC}"
            echo -e "${YELLOW}docker run -d --name $POSTGRES_CONTAINER -p $DB_PORT:5432 -e POSTGRES_PASSWORD=PASS -e POSTGRES_DB=ai_ethical_dm pgvector/pgvector:pg17${NC}"
        else
