#!/usr/bin/env python3
"""
FINAL Optimized Prediction Service with Enhanced Ontology Utilization and FIXED Extraction.
...
class OptimizedPredictionService(PredictionService):
    """Enhanced prediction service with optimized ontology utilization and FIXED extraction."""
    ...
    def generate_conclusion_prediction(self, document_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            # Extract just the conclusion from the response using FIXED method
            conclusion = self._extract_conclusion_fixed(response)
    ...
    def _extract_conclusion_fixed(self, response_obj) -> str:
        """
        FIXED: Extract conclusion part from the LLM response with proper regex.
        ...
        # FIXED: Use improved pattern that captures everything after CONCLUSION
        import re
        
        # Try pattern that captures everything after CONCLUSION header
        conclusion_match = re.search(r'(?i)#*\s*CONCLUSION:?\s*(.*)', response, re.DOTALL)
        
        if conclusion_match:
            extracted = conclusion_match.group(1).strip()
            logger.info(f"✅ FIXED extraction successful: {len(extracted)} characters")
            return extracted
        
        # Fallback: If response starts with conclusion-like content, return whole response
        if response.strip().lower().startswith('#') and 'conclusion' in response.lower()[:100]:
            logger.info(f"✅ Using fallback extraction: {len(response)} characters")
            return response.strip()
        
        # Last resort: return whole response
        logger.warning(f"⚠️ Using whole response as fallback: {len(response)} characters")
        return response
...
def test_optimization():
    ...
    with app.app_context():
        try:
            print("🎯 TESTING FINAL OPTIMIZED ONTOLOGY INTEGRATION")
            print("="*60)
            ...
            if result.get('success'):
                print("✅ FINAL optimized prediction generated successfully!")
                ...
                if metadata:
                    ...
                    if validation:
                        ...
                        print(f"Direct mentions: {validation.get('direct_mentions', 0)}")
                        print(f"Semantic mentions: {validation.get('semantic_mentions', 0)}")
                        mention_ratio = validation.get('mention_ratio', 0)
                        print(f"Mention ratio: {mention_ratio:.1%}")
                        print(f"Status: {validation.get('validation_status', 'unknown')}")
                        print(f"🎯 Optimization success: {validation.get('optimization_success', False)}")
                        
                        if mention_ratio >= 0.20:
                            print(f"🎉 SUCCESS! Achieved target >20% mention ratio!")
                        else:
                            print(f"📈 Improvement needed to reach >20% target")
                
                # Show prediction sample
                prediction = result.get('prediction', '')
                print(f"\n📝 FINAL OPTIMIZED PREDICTION SAMPLE (first 500 chars):")
                print(prediction[:500] + "...")
                
                return result
            else:
                print(f"❌ Final optimization test failed: {result.get('error')}")
                return None
                
        except Exception as e:
            print(f"❌ Error in final optimization test: {e}")
            import traceback
            traceback.print_exc()
            return None
