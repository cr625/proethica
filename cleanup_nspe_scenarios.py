#!/usr/bin/env python3
"""
Script to clean up incorrectly created NSPE scenarios.
...
def cleanup_nspe_scenarios():
    ...
    with app.app_context():
        ...
        # Confirm removal
        confirm = input("Are you sure you want to remove these scenarios? (y/n): ")
        if confirm.lower() != 'y':
        ...
        for scenario in scenarios_to_remove:
            try:
                ...
                EntityTriple.query.filter_by(scenario_id=scenario.id).delete()
                
                # Delete the scenario
                db.session.delete(scenario)
                
                print(f"Deleted scenario ID {scenario.id}: {scenario.name}")
