document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleHopsControl() {
        ...
        const hopsCtrl = document.getElementById('hops-control');
        if (caseId) {
            hopsCtrl.style.cssText = '';
        } else {
            hopsCtrl.style.display = 'none';
            hopsCtrl.style.setProperty('display', 'none', 'important');
        }
    }
    ...
    function loadGraph() {
        ...
        if (caseId) {
            url += `case_id=${caseId}&hops=${currentHops}&`;
        }
            ...
            .then(data => {
                ...
                setTimeout(() => {
                    ...
                    if (caseId) {
                        ...
                        if (focusNode) {
                            showNodeDetails({stopPropagation: () => {}}, focusNode);
                        }
                    }
                }, 50);
            })
    ...
    function renderGraph(data) {
        ...
        // Scale forces by node count: fewer nodes get stronger repulsion,
        // weaker centering, and wider collision to fill the space
        ...
        function fitToView() {
            ...
            // Compute bounds from node positions (not SVG getBBox,
            // which includes gridlines spanning the full canvas width)
            const pad = d3.max(nodes, n => radiusScale(n.in_degree)) + 20;
            const xExtent = d3.extent(nodes, n => n.x);
            const yExtent = d3.extent(nodes, n => n.fy !== undefined ? n.fy : n.y);
            const bx = xExtent[0] - pad;
            const by = yExtent[0] - pad;
            const bw = (xExtent[1] - xExtent[0]) + pad * 2;
            const bh = (yExtent[1] - yExtent[0]) + pad * 2;
            if (bw === 0 || bh === 0) return;
            ...
            const tx = (fullWidth - bw * scale) / 2 - bx * scale;
            const ty = (fullHeight - bh * scale) / 2 - by * scale;
    ...
    document.getElementById('hops-minus').addEventListener('click', function() {
        if (currentHops > MIN_HOPS) {
            currentHops--;
            updateHopsDisplay();
            loadGraph();
        }
    ...
    document.getElementById('hops-plus').addEventListener('click', function() {
