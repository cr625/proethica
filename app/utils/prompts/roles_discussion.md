...
=== DISCUSSION PASS (building on the Facts pass) ===
This is the DISCUSSION pass. The roles already extracted from the case FACTS section are included in the
"EXISTING ROLES IN ONTOLOGY" list at the top of this prompt. The Facts pass runs first and its roles are
the starting point for this pass. Therefore:
- REUSE an existing role identity (via match_decision) whenever the discussion refers to the same party or
  role type already extracted from the facts. Do NOT re-extract it as a new class or individual.
- Extract a role ONLY when it is genuinely NEW in the discussion (a party or role type not present in the
  facts), or when the discussion CLARIFIES or REFINES a facts-pass role in a way that changes its class or
  relationship category.
- The discussion typically analyzes the SAME actors introduced in the facts; prefer matching and refining
  over duplicating. A near-duplicate of a facts-pass role is an error.
