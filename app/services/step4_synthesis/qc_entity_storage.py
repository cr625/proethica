...
(``entity_uri``/``extraction_model``/``ontology_target``) and/or emit different
``rdf_json_ld`` keys, so sharing these builders with them would change emitted
output. These factories return an unsaved row; the caller does
``db.session.add(...)`` and commits, exactly as the inline loops did.
"""
