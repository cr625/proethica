...
Coordinates sub-phase execution for Step 4 complete synthesis:
  - clear_step4_data: Remove Phase 2-4 entities before re-extraction
  - run_provisions: 2A code provision extraction
  - run_precedents: 2B precedent case extraction
  - run_qc_unified: 2C question + conclusion extraction and linking
  - run_transformation: 2D transformation classification
  - run_rich_analysis: 2E causal links, question emergence, resolution patterns
  - run_phase3: Phase 3 decision point synthesis
  - run_phase4: Phase 4 narrative construction

All functions take a case_id (and optionally an llm_client) and return
a result dict.  They require a Flask app context for SQLAlchemy access
but do NOT depend on Flask request globals (request, session, g).
"""
...
# NOTE: step4 config and helpers are imported lazily inside each function
# to avoid a circular import chain:
#   step4_orchestration_service -> step4.config -> step4/__init__ -> run_all -> step4_orchestration_service
...
def clear_step4_data(case_id: int) -> dict:
    """Clear all Step 4 data (Phase 2-4) while preserving Phase 1 entities.

    Must stay in sync with clear_step4_data() in step4.py.
    """
    from app.routes.scenario_pipeline.step4.config import reset_step4_case_features
...
def run_provisions(case_id: int, llm_client) -> dict:
    """Run provisions extraction -- same logic as extract_provisions_streaming."""
...
def run_precedents(case_id: int, llm_client) -> dict:
    """Run precedent case extraction -- same logic as extract_precedents_streaming."""
    from app.routes.scenario_pipeline.step4.config import STEP4_SECTION_TYPE, STEP4_DEFAULT_MODEL
...
# ---------------------------------------------------------------------------
# 2C: Questions & Conclusions (unified)
# ---------------------------------------------------------------------------

def run_qc_unified(case_id: int, llm_client) -> dict:
    """Run Q&C unified extraction -- same logic as extract_qc_unified_streaming."""
    from app.routes.scenario_pipeline.step4.config import STEP4_SECTION_TYPE, STEP4_DEFAULT_MODEL
    from app.routes.scenario_pipeline.step4.helpers import get_all_case_entities
...
def run_transformation(case_id: int, llm_client) -> dict:
    """Run transformation classification -- same logic as extract_transformation_streaming."""
    from app.routes.scenario_pipeline.step4.config import STEP4_SECTION_TYPE, STEP4_DEFAULT_MODEL
    from app.routes.scenario_pipeline.step4.helpers import get_all_case_entities
...
def run_rich_analysis(case_id: int) -> dict:
...
def run_phase3(case_id: int) -> dict:
    """Run Phase 3 decision synthesis -- same logic as synthesize_phase3_streaming."""
    from app.routes.scenario_pipeline.step4.config import STEP4_SECTION_TYPE, STEP4_DEFAULT_MODEL
...
def run_phase4(case_id: int) -> dict:
    """Run Phase 4 narrative construction -- same logic as construct_phase4_streaming."""
    from app.routes.scenario_pipeline.step4.config import STEP4_SECTION_TYPE, STEP4_DEFAULT_MODEL
    ...
    try:
        ...
        if transformation_record and transformation_record.results_summary:
            try:
                ...
                logger.warning(f"[RunAll] Could not parse transformation_type from results_summary: {e}")
