...
class GuidelineAnalysisService:
    ...
    def extract_ontology_terms_from_text(self,
        ...
        This is Stage 2 - finds mentions of engineering-ethics ontology terms in the text itself.
        Completely separate from concept extraction - looks for roles, capabilities, conditions, etc.
        ...
        try:
            ...
            # Simple fallback implementation - just look for common terms
            # In production, this would use the MCP server or LLM
            common_terms = {
                'roles': ['engineer', 'client', 'employer', 'public', 'colleague'],
                'actions': ['design', 'review', 'approve', 'report', 'disclose'],
                'conditions': ['conflict of interest', 'safety risk', 'ethical dilemma'],
                'capabilities': ['competence', 'expertise', 'qualification', 'skill'],
                'principles': ['integrity', 'honesty', 'safety', 'welfare', 'sustainability']
            }
            
            # Convert text to lowercase for matching
            text_lower = guideline_text.lower()
            
            # Find mentions of each term category
            for category, terms in common_terms.items():
                for term in terms:
                    if term in text_lower:
                        # Create triple for this mention
                        term_uri = f"{ontology_namespace}{self._slugify(term)}"
                        ...
                        mention_triple = {
                            ...
                            'object_uri': term_uri,
                            'object_label': term.title(),
                            'triple_metadata': {
                                'category': category,
                                'found_in_text': True
                            }
                        }
                        extracted_triples.append(mention_triple)
                        ...
                        if category == 'roles':
                            ...
                            extracted_triples.append(role_triple)
                        elif category == 'actions':
                            action_triple = {
                                'subject': guideline_uri,
                                'subject_label': f'Guideline {guideline_id}',
                                'predicate': f"{ontology_namespace}describesAction",
                                'predicate_label': 'describes action',
                                'object_uri': term_uri,
                                'object_label': term.title()
                            }
                            extracted_triples.append(action_triple)
                        elif category == 'principles':
                            principle_triple = {
                                'subject': guideline_uri,
                                'subject_label': f'Guideline {guideline_id}',
                                'predicate': f"{ontology_namespace}emphasizesPrinciple",
                                'predicate_label': 'emphasizes principle',
                                'object_uri': term_uri,
                                'object_label': term.title()
                            }
            ...
            logger.exception(f"Error extracting ontology terms from text: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'triples': [],
    ...
    def _calculate_semantic_similarity(self, text1: str, text2: str) -> float:
        ...
        return jaccard * 0.7  # Scale to max 0.7 for word overlap
    
    def _suggest_ontology_parent(self, concept: Dict[str, Any], concept_type: str) -> Optional[Dict[str, str]]:
        """
        Suggest an appropriate parent class in the ontology for a new concept.
        """
        # Default parent classes by type
        parent_mapping = {
            'principle': {
                'uri': 'http://proethica.org/ontology/Principle',
                'label': 'Principle'
            },
            'obligation': {
                'uri': 'http://proethica.org/ontology/Obligation',
                'label': 'Obligation'
            },
            'role': {
                'uri': 'http://proethica.org/ontology/Role',
                'label': 'Role'
            },
            'action': {
            ...
            'condition': {
                'uri': 'http://proethica.org/ontology/Condition',
                'label': 'Condition'
            },
            'capability': {
        ...
        return parent_mapping.get(concept_type)
