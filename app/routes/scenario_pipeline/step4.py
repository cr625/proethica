...
def clear_step4_data(case_id):
    ...
    try:
        ...
        if activity_ids:
            # Find entity IDs generated by these activities
            step4_entity_ids = [
            ...
            # Delete usage records FIRST (FK: usage.entity_id -> entities)
            ProvenanceUsage.query.filter(
                ProvenanceUsage.activity_id.in_(activity_ids)
            ).delete(synchronize_session=False)

            if step4_entity_ids:
                # Delete derivations (FK: derivation.source/derived_entity_id -> entities)
