...
def generate_completion_code() -> str:
    ...
    moment the participant finishes (retrospective submit) and is what they
    paste into Prolific.
    """
    return ''.join(secrets.choice(CODE_ALPHABET) for _ in range(CODE_LENGTH))
...
def admin_dashboard():
    ...
    (attention-check pass rate, retrospective completion).
...
def preview_start():
    ...
    #   complete      — final completion page (also pre-stamps ranking row + code)
