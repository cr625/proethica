#!/usr/bin/env python3
"""
Complete Schema Fix for ProEthica Categories

Recreates all ProEthica 9-category tables with schemas that EXACTLY match
the SQLAlchemy models field-for-field.
"""
...
def fix_schema():
    ...
    try:
        ...
        logger.info("Connected to database, recreating all ProEthica category tables...")
        
        # Drop all category tables first
        tables_to_drop = ['constraints', 'capabilities', 'states', 'obligations', 'principles']  # Order matters due to foreign keys
        for table in tables_to_drop:
            logger.info(f"Dropping {table} table...")
            cursor.execute(f"DROP TABLE IF EXISTS {table} CASCADE;")
        
        # Create Principles table (EXACT match to Principle model)
        logger.info("Creating principles table (exact model match)...")
        cursor.execute("""
        ...
        # Create Obligations table (EXACT match to Obligation model)
        logger.info("Creating obligations table (exact model match)...")
        cursor.execute("""
        ...
        # Create States table (EXACT match to State model)
        logger.info("Creating states table (exact model match)...")
        cursor.execute("""
            CREATE TABLE states (
                id SERIAL PRIMARY KEY,
                scenario_id INTEGER NOT NULL REFERENCES scenarios(id) ON DELETE CASCADE,
                ...
                changed_by_action_id INTEGER,
                previous_value VARCHAR(255),
                bfo_class VARCHAR(255) DEFAULT 'BFO_0000019',
                proethica_category VARCHAR(50) DEFAULT 'state',
                ontology_uri VARCHAR(500),
                extraction_confidence FLOAT DEFAULT 0.0,
                extraction_method VARCHAR(100) DEFAULT 'llm_enhanced',
                validation_notes TEXT,
                observed_at_event_id INTEGER,
            ...
            CREATE INDEX idx_states_value ON states(value);
            GRANT ALL PRIVILEGES ON TABLE states TO proethica_user;
            GRANT ALL PRIVILEGES ON SEQUENCE states_id_seq TO proethica_user;
        """)
        
        # Create Capabilities table (EXACT match to Capability model)
        logger.info("Creating capabilities table (exact model match)...")
        cursor.execute("""
        ...
        # Create Constraints table (EXACT match to Constraint model)
        logger.info("Creating constraints table (exact model match)...")
        cursor.execute("""
        ...
        cursor.execute("""
            ...
            AND column_name IN ('source', 'severity', 'value')
            ORDER BY table_name, column_name;
        """)
        
        verification = cursor.fetchall()
        logger.info("Verification - Critical columns:")
