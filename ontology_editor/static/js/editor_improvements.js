/**
 * Improved Ontology Editor JavaScript
 * 
 * Enhancements:
 * 1. Better URL parameter handling with History API
 * 2. Improved version display and management
 * 3. More consistent navigation between ontologies
 */

// Initialize global variables
let editor;               // ACE editor instance
let currentOntologyId;    // Currently loaded ontology ID
let currentVersionId;     // Currently loaded version ID
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (viewMode === 'full') {
        document.querySelector('.container-fluid').classList.add('full-width-mode');
    }
    
    // Always load the list of ontologies for the sidebar
    loadOntologyList();
    
    // Initialize the version dropdown in the toolbar
    ...
    window.addEventListener('popstate', function(event) {
        if (event.state && event.state.ontologyId) {
            loadOntology(event.state.ontologyId, true); // Second param prevents adding to history again
        }
    });
});
...
function initializeVersionDropdown() {
    // Add the version dropdown to the editor toolbar
    const editorTitle = document.getElementById('editorTitle');
    const versionDropdownHtml = `
        <div class="btn-group ms-3 version-dropdown" id="versionToolbarDropdown" style="display: none;">
            <button class="btn btn-sm btn-outline-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown">
                <span id="currentVersionDisplay">Current Version</span>
            </button>
            <ul class="dropdown-menu" id="versionToolbarMenu">
    ...
    editorTitle.insertAdjacentHTML('afterend', versionDropdownHtml);
}
...
function initializeEditor() {
    ...
    editor.getSession().on('change', function() {
        if (!isEditorDirty && currentOntologyId) {
            ...
            // Indicate modified content in the version dropdown
            const currentVersionDisplay = document.getElementById('currentVersionDisplay');
            if (currentVersionDisplay && !currentVersionDisplay.innerText.includes('*')) {
                currentVersionDisplay.innerText += ' *';
            }
        }
    });
}
...
function setupEventListeners() {
    ...
    document.getElementById('visualizeBtn').addEventListener('click', function() {
        if (currentOntologyId) {
            // Use state to preserve current parameters
            const urlParams = new URLSearchParams(window.location.search);
            const state = { 
...
function loadOntologyBySource(source) {
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            ...
            // Update URL using History API (without reloading the page)
            // This replaces source param with ontology_id
            updateUrlWithoutRefresh(currentOntologyId);
        ...
        .catch(error => {
            ...
            document.getElementById('versionToolbarDropdown').style.display = 'none';
...
function updateOntologyList(ontologies) {
    ...
    const items = ontologies.map(ontology => {
        ...
        const isActive = currentOntologyId && ontology.id == currentOntologyId;
        
        return `
            <li class="list-group-item ${isActive ? 'active-ontology' : ''}" 
...
/**
 * Update the URL using the History API without refreshing the page
 * 
 * @param {string|number} ontologyId - ID of the current ontology
 * @param {boolean} replace - Whether to replace the current history entry instead of adding a new one
 */
function updateUrlWithoutRefresh(ontologyId, replace = false) {
    if (!ontologyId) return;
    
    // Create a new URL based on the current one
    const newUrl = new URL(window.location.href);
    
    // Update or add ontology_id parameter
    newUrl.searchParams.set('ontology_id', ontologyId);
    
    // Remove source parameter if it exists (replaced by ontology_id)
    ...
    // Create state object to use with History API
    const state = { 
        ontologyId: ontologyId,
        view: viewParam 
    };
    
    // Update browser history (either push new entry or replace current)
    if (replace) {
 ...
 */
function loadOntology(ontologyId, skipHistory = false) {
    ...
    fetch(`/ontology-editor/api/ontology/${ontologyId}`)
        ...
        .then(data => {
            ...
            // Update URL using History API (unless skipping)
            if (!skipHistory) {
                updateUrlWithoutRefresh(ontologyId);
            }
...
function updateUIForLoadedOntology(ontologyId) {
    ...
    // Reset version dropdown display
    document.getElementById('currentVersionDisplay').innerText = 'Latest Version';
}
...
function loadVersions(ontologyId) {
    fetch(`/ontology-editor/api/versions/${ontologyId}`)
        ...
        .then(data => {
            ...
            updateVersionToolbarDropdown(data.versions || []);
        })
        .catch(error => {
...
/**
 * Update the versions list in the UI sidebar
 * 
...
function updateVersionsList(versions) {
    ...
    const items = versions.map((version, index) => {
        ...
        const isActive = currentVersionId && version.id == currentVersionId;
        
        return `
            <li class="list-group-item ${isActive ? 'active' : ''}" data-version-id="${version.id}">
...
/**
 * Update the version dropdown in the toolbar
 * 
 * @param {Array} versions - List of version metadata objects
 */
function updateVersionToolbarDropdown(versions) {
    const dropdownMenu = document.getElementById('versionToolbarMenu');
    const versionDropdown = document.getElementById('versionToolbarDropdown');
    ...
    const items = versions.map((version, index) => {
        ...
        return `
            <li>
                <a class="dropdown-item version-dropdown-item" href="#" data-version-id="${version.id}">
                    <div class="d-flex justify-content-between">
                        <span>v${version.version_number} ${isLatest ? '(Latest)' : ''}</span>
                        <small class="text-muted">${formattedDate}</small>
                    </div>
                    ${version.commit_message ? 
                        `<small class="text-truncate d-block">${version.commit_message}</small>` : ''}
                </a>
            </li>
        `;
    }).join('');
    ...
    if (currentVersionId) {
        ...
        if (currentVersion) {
            document.getElementById('currentVersionDisplay').innerText = 
...
function loadVersion(versionId) {
    ...
    fetch(`/ontology-editor/api/versions/${versionId}`)
        ...
        .then(data => {
            ...
            updateVersionDisplays(versionId);
...
/**
 * Update all version displays when a version is loaded
 * 
 * @param {string} versionId - ID of the loaded version
 */
function updateVersionDisplays(versionId) {
    // Highlight the selected version in the sidebar
    document.querySelectorAll('#versionList li').forEach(item => {
    ...
    // Update the version dropdown in the toolbar
    fetch(`/ontology-editor/api/version-info/${versionId}`)
        .then(response => {
            if (!response.ok) {
        ...
        .catch(error => {
            ...
            document.getElementById('currentVersionDisplay').innerText = 'Selected Version';
        });
}
...
function validateOntology() {
    ...
    .then(syntaxResult => {
        ...
        // Display success message for syntax validation
 ...
 * @param {Object} results - Additional validation results
 */
function displayValidationResults(syntaxValid, results) {
    const resultsElement = document.getElementById('validationResults');
    let html = '';
    ...
    // BFO compliance
    html += '<div class="mb-3"><strong>Ontology Compliance:</strong></div>';
    ...
    if (results.warnings && results.warnings.length > 0) {
        ...
        html += '<div class="validation-success">No compliance issues found</div>';
    }
...
function saveOntology() {
    ...
    .then(data => {
        // Reset current version ID (we're now on the latest version)
        currentVersionId = null;
