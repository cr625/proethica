#!/bin/bash
# Fixed starter script for using the modified codespace environment
# This script has simplified database checking to avoid hanging

# Print banner
echo "=== ProEthica Codespace Simple Starter (Fixed) ==="
...
if [ -z "$CONTAINER_ID" ]; then
    ...
    docker exec postgres17-pgvector-codespace psql -U postgres -c "CREATE DATABASE ai_ethical_dm;" || true
else
    echo "PostgreSQL container is already running with ID: $CONTAINER_ID"
fi

# Quick database check without hanging
echo "Testing database connection (quick check)..."
PGPASSWORD=postgres psql -h localhost -p 5433 -U postgres -c "SELECT 1;" > /dev/null 2>&1
if [ $? -eq 0 ]; then
...
timeout 5 curl -s -X POST http://localhost:5001/jsonrpc \
     -H "Content-Type: application/json" \
     -d '{"jsonrpc":"2.0","method":"list_tools","params":{},"id":1}' > /dev/null || {
