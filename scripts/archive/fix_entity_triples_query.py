#!/usr/bin/env python3
"""
Fixed Entity Triples Query
-----------------------
This script correctly queries triples for a case using the subject URI pattern
instead of the temporal_region_type field, since the debug output shows that
our ontology integration is storing triples with the subject URI pattern
but not setting the temporal_region_type field.
"""
...
def query_triples_for_case(case_id):
    """
    Query all triples for a specific case ID using the subject URI pattern
    instead of temporal_region_type.
    """
    conn = get_connection()
    if not conn:
        return []
    ...
    # The correct way to find case triples - looking for subject pattern
    case_uri = f"http://proethica.org/cases/{case_id}"
    cursor.execute("""
        SELECT id, subject, predicate, object_uri, object_literal, is_literal, 
               graph, triple_metadata, entity_type, temporal_region_type
        FROM entity_triples
        WHERE subject = %s
...
def save_triples_to_file(case_id):
    ...
    with open(output_file, 'w') as f:
        ...
        for triple in triples:
            id, subject, predicate, object_uri, object_literal, is_literal, graph, metadata, entity_type, temp_region_type = triple
            ...
            triple_obj = {
                ...
                "temporal_region_type": temp_region_type
            }
        ...
        for graph, graph_triples in graphs.items():
            ...
            for triple in graph_triples:
                ...
                f.write(f"    Entity Type: {triple['entity_type']}\n")
                f.write(f"    Temporal Region Type: {triple['temporal_region_type']}\n")
    ...
    for graph, graph_triples in graphs.items():
        ...
        print(f"  {graph_name}: {len(graph_triples)} triples")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print("Usage: python fix_entity_triples_query.py <case_id>")
