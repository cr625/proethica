...
# Test extracting concepts from a guideline - using the correct module-prefixed method
echo "2. Testing guideline_analysis.extract_guideline_concepts..."
extract_response=$(curl -s -X POST http://localhost:5001/jsonrpc \
     -H "Content-Type: application/json" \
     -d '{"jsonrpc":"2.0","method":"guideline_analysis.extract_guideline_concepts","params":{"content":"Engineers shall hold paramount the safety, health, and welfare of the public."},"id":2}')
...
if [ $? -eq 0 ]; then
    ...
    echo "Response:"
    echo "$extract_response" | python -m json.tool | head -n 20
